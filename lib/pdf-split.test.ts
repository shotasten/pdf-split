...
describe("PDF分割コア", () => {
  ...
  it("回転角を正規化し、見た目の矩形をPDF座標へ変換する", () => {
    ...
    expect(mapVisualRectToPageBox({ x: 0, y: 0, width: 300, height: 600 }, { x: 10, y: 20, width: 600, height: 300 }, 180)).toEqual({ x: 310, y: 20, width: 300, height: 600 });
  });
  ...
  it("各ページを2分割し、5種のページボックスを分割範囲に揃える", async () => {
    ...
    const outputBytes = await splitPdfWithPageBoxes(await source.save(), "vertical", 50, "natural");
    ...
    expect(output.getPage(0).getCropBox()).toEqual({ x: 50, y: 40, width: 350, height: 450 });
  });

  it("中央吸着とダウンロード名の仕様を守る", () => {
    expect(snapToCenter(48.5)).toBe(50);
    expect(snapToCenter(52)).toBe(52);
    expect(buildOutputFileName("book.PDF")).toBe("book.PDF-split.pdf");
